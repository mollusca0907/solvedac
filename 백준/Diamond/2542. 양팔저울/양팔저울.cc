#include <bits/stdc++.h>
using namespace std;
using ld = long double;

constexpr ld EPS = 1e-12;

struct Point {
    ld x, y;

    Point operator+(Point other) { return { x + other.x, y + other.y }; }
    Point operator-(Point other) { return { x - other.x, y - other.y }; }
    Point operator*(ld k) { return { x * k, y * k }; }
};

ld cross(Point a, Point b) {
    //0보다크면왼쪽, 0보다작으면오른쪽
    return a.x * b.y - b.x * a.y; //외적이긴 한데 한점을 원점으로 옮겨버려서 간단해짐
}

struct Line {
    Point p, v; //직선을 한 점 p와 방향벡터 v로 표현
    ld angle;

    Line(Point p1, Point p2) { //p1->p2직선
        p = p1;
        v = p2 - p1;
        angle = atan2l(v.y, v.x); //아크탄젠트함수로 각 구하기
    }

    Line(ld a, ld b, ld c) {
        if (abs(a) > abs(b)) p = { -c / a, 0 };
        else p = { 0, -c / b };
        v = { b, -a };
        angle = atan2l(v.y, v.x);
    }

    bool in(Point other) {
        return cross(v, other - p) >= -EPS;
    }

};

Point crossPoint(Line a, Line b) {
    ld s = cross(b.v, a.p - b.p) / cross(a.v, b.v);
    return a.p + a.v * s;
}

vector<Point> HPI(vector<Line>& lines) {

    sort(lines.begin(), lines.end(), [](Line a, Line b) {
        if (abs(a.angle - b.angle) > EPS) return a.angle < b.angle;
        return cross(a.v, b.p - a.p) < -EPS;
        });

    deque<Line>dq;
    for (int i = 0; i < lines.size(); i++) {
        while (dq.size() >= 2 && !lines[i].in(crossPoint(dq[dq.size() - 2], dq[dq.size() - 1]))) {
            dq.pop_back();
        }
        while (dq.size() >= 2 && !lines[i].in(crossPoint(dq[0], dq[1]))) {
            dq.pop_front();
        }
        dq.push_back(lines[i]);
    }
    while (dq.size() >= 3 && !dq[0].in(crossPoint(dq[dq.size() - 2], dq[dq.size() - 1]))) {
        dq.pop_back();
    }
    while (dq.size() >= 3 && !dq[dq.size()-1].in(crossPoint(dq[0], dq[1]))) {
        dq.pop_front();
    }
    
    if (dq.size() < 3) return { };

    vector<Point> ans;
    for (int i = 0; i < dq.size(); i++) {
        ans.push_back(crossPoint(dq[i], dq[(i + 1) % dq.size()]));
    }

    return ans;

}

void parse(string s, ld& la, ld& lb, ld& lc, ld& ra, ld& rb, ld& rc, char& op) {
    la = lb = lc = ra = rb = rc = 0;
    bool second = false;

    for (int i = 0; i < s.size(); i++) {
        char ch = s[i];
        if (ch == ' ' || ch == '$') continue;

        if (ch == '<' || ch == '>' || ch == '|') {
            op = ch;
            second = true;
            continue;
        }
    
        if (!second) {
            if (ch == 'A') la++;
            else if (ch == 'B') lb++;
            else if (ch == '1') lc++;
        }
        else {
            if (ch == 'A') ra++;
            else if (ch == 'B') rb++;
            else if (ch == '1') rc++;
        }
    
    }
}

int main() {



    int k;
    cin >> k;
    string command;
    getline(cin, command);

    vector<Line> lines;
    
    lines.push_back(Line({ 1e9, 1e9 }, { EPS, 1e9 })); //상
    lines.push_back(Line({ EPS, EPS }, { 1e9, EPS })); //하
    lines.push_back(Line({ EPS, 1e9 }, { EPS, EPS })); //좌
    lines.push_back(Line({ 1e9, EPS }, { 1e9, 1e9 })); //우

    for (int i = 0; i < k; i++) {
        getline(cin, command);
        ld la, lb, lc, ra, rb, rc;
        char op;
        parse(command, la, lb, lc, ra, rb, rc, op);
        if (op == '>') {
            lines.push_back(Line(la - ra, lb - rb, lc - rc));
        }
        else {
            lines.push_back(Line(ra - la, rb - lb, rc - lc));
        }
    }

    auto pts = HPI(lines);

    for (int i = 0; i < 5; i++) {
        getline(cin, command);
        ld la, lb, lc, ra, rb, rc;
        char op;
        parse(command, la, lb, lc, ra, rb, rc, op);

        ld da = la - ra;
        ld db = lb - rb;
        ld dc = lc - rc;

        bool plus = false;
        bool minus = false;

        for (auto p : pts) {
            ld val = da * p.x + db * p.y + dc;
            if (val > EPS) plus = true;
            if (val < -EPS) minus = true;

        }

        if (plus && minus) {
            cout << "?\n";
        }
        else if (!plus && minus) {
            cout << "<\n";
        }
        else if (plus && !minus) {
            cout << ">\n";
        }

    }



    return 0;
}